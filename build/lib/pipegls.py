...
import os
import geohash
from nlgeojson import get_first_bounds
...
def make_type(data,filename,type):
	if type == 'points':
		mk.make_points(data,filename)
	if type == 'blocks':
		mk.make_blocks(data,filename)
	if type == 'lines':
		mk.make_lines(data,filename)
	if type == 'polygons':
		mk.make_polygons(data,filename)
...
add(map);
	...
	with open('index.html','wb') as f: f.write(html)
...
def eval_config(configdict):
  mk.cln()
  count = 1
  latlng = 0
  print len(configdict)
